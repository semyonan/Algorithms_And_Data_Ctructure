...
node* insert_and_balance(node *root, int k) {
    if (root == NULL) {
        node *tmp = (node*)malloc(sizeof(node));
        tmp->key = k;
        tmp->left = NULL;
    ...
    if (k < root->key) {
        root->left = insert_and_balance(root->left, k);
    } else if (k > root->key) {
        root->right = insert_and_balance(root->right, k);
    }
    return balance(root);
}
...
int main() {
    ...
    FILE *fin, *fout;
    fin = fopen("addition.in", "r");
    fout = fopen("addition.out", "w");
    ...
    fscanf(fin, "%d", &val);
    root = insert_and_balance(root, val);
    fprintf(fout, "%d\n", n+1);
